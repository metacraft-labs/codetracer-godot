# GDH5 probe, NO-STATICS control — VERSION 1.
#
# The control arm of gdh5_unpreserved_state_is_reported: this script has no
# static variable at all, so a reload of it must report NO loss. A host that
# reported one unconditionally — the `oldCodeRetained: true` shape — is red
# here and green on probe_v1.gd, which is what makes the report a
...
func probe(n: int) -> void:
	print("GDH5_V1_A tick=", n)
	print("GDH5_V1_B tick=", n)
	print("GDH5_V1_C tick=", n)
