...
# Constants & Enums) record-and-verify runner.
#
# Builds the patched engine if needed, records the GF1 reference program
# (gf_typing.gd), decodes its .ct with ct-print --full, and asserts the
# hand-derived facts in scripts/EXPECTED-GF1.md via scripts/verify_gf1.py:
# every named local carries the correct NAME + VALUE + type-kind on the step at
# its own source line, across the full typing/operator/const/enum surface
# (values.dat parallel-indexed to steps.dat). The static-var write is asserted
# ABSENT (deferred to GF8, member writes).
#
# GF1 adds NO new VM hook: the `var name = <expr>` codegen routes every
# operator / is / as / in / ternary / const / enum result through an
# OPCODE_ASSIGN* into the named slot, which G4 already hooks. This runner proves
# the existing capture pipeline is correct across that surface.
#
# Then it PROVES the verifier has teeth with three tamper runs (wrong value /
# wrong type-kind / wrong step), each of which MUST be rejected.
#
# Finally it re-runs the G4 value + G3 call/return + G2 step regressions to
# confirm nothing was perturbed.
#
# EXITS NONZERO on any mismatch. This is a real automated test, not a print.
#
# Usage:
#   scripts/record-and-verify-gf1.sh              # build-if-needed, then verify
#   BUILD=never scripts/record-and-verify-gf1.sh  # fail if binary missing
#   BUILD=always scripts/record-and-verify-gf1.sh # force a rebuild first
set -euo pipefail
...
# --- 2. work dir -----------------------------------------------------------
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ct-gf1.XXXXXX")"
...
# --- 3. GF1 deliverable: captured typing/operators/const/enums -------------
log "recording gf_typing.gd (GF1 typing/operators/const/enums)"
...
python3 "$REPO/scripts/verify_gf1.py" verify "$GF1_FULL" \
	|| die "gf_typing.gd: verify_gf1.py assertions failed"

# --- 4. prove the verifier has teeth (tamper runs MUST be rejected) --------
log "tamper runs (each MUST be rejected by verify_gf1.py)"
for mode in value kind step; do
	python3 "$REPO/scripts/verify_gf1.py" tamper "$GF1_FULL" "$mode" \
		|| die "tamper($mode) was NOT rejected — verifier is vacuous"
done

# --- 5. regression: G4 values + G3 calls/returns + G2 steps unchanged ------
log "regression: G4 (gf_values.gd) + G3 (gf_calls.gd) + G2 (g2probe.gd) unchanged"
...
log "ALL GF1 + tamper + G4/G3/G2-regression checks PASSED"
