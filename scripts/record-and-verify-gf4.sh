...
# math / struct / handle types) record-and-verify runner.
#
# Builds the patched engine if needed, records the GF4 reference program
# (gf_variant_types.gd), decodes its .ct with ct-print --full, and asserts the
# hand-derived facts in scripts/EXPECTED-GF4.md via scripts/verify_gf4.py:
#   - each math/struct type -> a Struct with the right registered type name and
#     positional field values (nested structs checked recursively);
#   - the handle types (StringName/NodePath -> String; RID/Callable/Signal/Object
#     -> shallow Struct; null -> None);
#   - the packed struct-arrays (PackedVector2/3Array, PackedColorArray) ->
#     Sequences of the matching struct element.
#
# GF4 CHANGES the recorder (the Variant->ct_value_* switch now encodes the math/
# struct/handle types instead of the G4/GF3 Raw fallback), so this runner
# rebuilds the engine.
#
# Then it PROVES the verifier has teeth with tamper runs (wrong field value /
# wrong kind / wrong type name / wrong nested shape), each of which MUST be
# rejected.
#
# Finally it re-runs the GF3 + GF2 + GF1 + G4 + G3 + G2 regressions to confirm the
# new struct encoding did not perturb collection/scalar capture or the step/call
# streams (scalar-only types tables stay the 6 base entries — lazy interning).
#
# EXITS NONZERO on any mismatch. This is a real automated test, not a print.
#
# Usage:
#   scripts/record-and-verify-gf4.sh              # build-if-needed, then verify
#   BUILD=never scripts/record-and-verify-gf4.sh  # fail if binary missing
#   BUILD=always scripts/record-and-verify-gf4.sh # force a rebuild first
set -euo pipefail
...
# --- 2. work dir -----------------------------------------------------------
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ct-gf4.XXXXXX")"
...
# --- 3. GF4 deliverable: math/struct/handle Variant capture ----------------
log "recording gf_variant_types.gd (GF4 math/struct/handle types)"
...
python3 "$REPO/scripts/verify_gf4.py" verify "$GF4_FULL" \
	|| die "gf_variant_types.gd: verify_gf4.py assertions failed"

# --- 4. prove the verifier has teeth (tamper runs MUST be rejected) --------
log "tamper runs (each MUST be rejected by verify_gf4.py)"
for mode in fieldval kind typename shape; do
	python3 "$REPO/scripts/verify_gf4.py" tamper "$GF4_FULL" "$mode" \
		|| die "tamper($mode) was NOT rejected — verifier is vacuous"
done

# --- 5. regression: GF3 + GF2 + GF1 + G4 + G3 + G2 unchanged ---------------
log "regression: GF3 (gf_collections.gd) + GF2 + GF1 + G4 + G3 + G2"
...
log "ALL GF4 + tamper + GF3/GF2/GF1/G4/G3/G2-regression checks PASSED"
