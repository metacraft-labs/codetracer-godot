#!/usr/bin/env bash
# CodeTracer GDScript recorder — GF3 (Collections: Arrays, Typed/Packed Arrays,
# Dictionaries, Typed Dictionaries) record-and-verify runner.
#
# Builds the patched engine if needed, records the GF3 reference program
# (gf_collections.gd), decodes its .ct with ct-print --full, and asserts the
# hand-derived facts in scripts/EXPECTED-GF3.md via scripts/verify_gf3.py:
#   - each collection's STRUCTURE + element values: Array/Packed*Array -> a
#     Sequence with the right element count and each element's value/type;
#     Dictionary -> a Sequence of key/value Tuples with the right keys+values;
#   - one level of nesting (array-of-arrays, dict-with-array-value);
#   - the post-mutation value (append changes length 3 -> 4).
#
# GF3 CHANGES the recorder (the Variant->ct_value_* switch now encodes
# collections structurally instead of the G4 Raw fallback), so this runner
# rebuilds the engine.
#
# Then it PROVES the verifier has teeth with tamper runs (wrong element value /
# wrong length / wrong dict key / wrong dict value / wrong nesting), each of
# which MUST be rejected.
#
# Finally it re-runs the GF2 + GF1 + G4 + G3 + G2 regressions to confirm the new
# compound encoding did not perturb scalar capture or the step/call streams.
#
# EXITS NONZERO on any mismatch. This is a real automated test, not a print.
#
# Usage:
#   scripts/record-and-verify-gf3.sh              # build-if-needed, then verify
#   BUILD=never scripts/record-and-verify-gf3.sh  # fail if binary missing
#   BUILD=always scripts/record-and-verify-gf3.sh # force a rebuild first
set -euo pipefail
...
# --- 2. work dir -----------------------------------------------------------
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ct-gf3.XXXXXX")"
...
# --- 3. GF3 deliverable: structured collection capture ---------------------
log "recording gf_collections.gd (GF3 collections)"
...
python3 "$REPO/scripts/verify_gf3.py" verify "$GF3_FULL" \
	|| die "gf_collections.gd: verify_gf3.py assertions failed"

# --- 4. prove the verifier has teeth (tamper runs MUST be rejected) --------
log "tamper runs (each MUST be rejected by verify_gf3.py)"
for mode in elem length dictkey dictval nesting; do
	python3 "$REPO/scripts/verify_gf3.py" tamper "$GF3_FULL" "$mode" \
		|| die "tamper($mode) was NOT rejected — verifier is vacuous"
done

# --- 5. regression: GF2 + GF1 + G4 + G3 + G2 unchanged ---------------------
log "regression: GF2 (gf_control_flow.gd) + GF1 (gf_typing.gd) + G4 (gf_values.gd) + G3 (gf_calls.gd) + G2 (g2probe.gd)"
...
log "ALL GF3 + tamper + GF2/GF1/G4/G3/G2-regression checks PASSED"
