...
# also answers it.
ct_gdh6_restore_bin_on_exit() {
  local rc=$?
  if [[ -f "$PLAIN" ]] && ! cmp -s "$BIN" "$PLAIN"; then
    if cp -f "$PLAIN" "$BIN"; then
      echo "[gdh6] restored the plain engine to $BIN on exit (it was armed)" >&2
    else
      echo "[gdh6] WARNING: could NOT restore the plain engine to $BIN — an" \
           "ARMED engine is left in the tree. Copy $PLAIN back by hand." >&2
    fi
  fi
  exit $rc
}
trap ct_gdh6_restore_bin_on_exit EXIT INT TERM
